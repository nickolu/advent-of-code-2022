...
function applyInstructionsToCrateMap(
  ...
  instructions.forEach((instruction) => {
    ...
    const fromStack = crateMap[fromColumn];
    const toStack = crateMap[toColumn];
    const moved = fromStack.splice(
      fromStack.length - Number(numberToMove),
      Number(numberToMove)
    );
    crateMap[toColumn] = toStack.concat(moved);
  });
